...
// N * 2 个控制量
size_t v_e_start = omega_start + N;
...
class MPC 
{
	public:
  	...
  	Solution Solve(Eigen::VectorXd x0, Eigen::VectorXd X_ref,
       Eigen::VectorXd Y_ref, Eigen::VectorXd Theta_ref, Eigen::VectorXd V_ref, Eigen::VectorXd control_out);
};
...
class FG_eval
{
  public:
    ...
    // 先取出多项式系数，和 预测的动作
    FG_eval(Eigen::VectorXd x0, Eigen::VectorXd X_ref,
                Eigen::VectorXd Y_ref, Eigen::VectorXd Theta_ref, Eigen::VectorXd V_ref, Eigen::VectorXd control_out) // 构造函数
    { 
        // 传入参考轨迹
        this->X_ref = X_ref;
        this->Y_ref = Y_ref;
    ...
    for (int i = 0; i <= N - 1; i++) 
    {
        ...
        AD<double> V_k = -W_e_k + V_ref[i] * CppAD::cos(Etheta_k);
