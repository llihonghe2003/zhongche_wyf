...
SequentialAuction::SequentialAuction(vector<Task> unallocated_tasks,
  ...
  // this->alloction_result(num_robots);
...
vector<vector<double> > SequentialAuction::allocateTasks() {
  int winning_robot, winning_task;
  prepareAllocations();
  calculateAllBids();
  while (!unalloc.empty()) {
    system("clear");
    cout << "本轮竞拍出价: " << endl << endl;

    printBids();
    // sleep(1);
    selectWinner(winning_robot, winning_task);
    // char a[50]<< "winner is: " << winning_robot << ", " << winning_task <<
    // endl;

    cout << "\n\t机器人: " << winning_robot << "   获得任务:" << winning_task
         << endl;
...
double SequentialAuction::insertTask(int robot_num, int unalloc_id,
                                     vector<int> &new_path) {
...
void SequentialAuction::printPaths() {
  system("clear");
  cout << "\n任务分配完成,最终分配结果:" << endl << endl;
  // sleep(0.5);
  alloction_result.resize(num_robots);
  for (int i = 0; i < num_robots; i++) {
    cout << "\t机器人" << i << ":";
    alloction_result[i].push_back(
        static_cast<double>(robot_poses[i].position.x));
    alloction_result[i].push_back(
        static_cast<double>(robot_poses[i].position.y));
    printPath(i, paths[i]);
  }
  // sleep(0.5);
  cout << "\n分配结果:" << alloction_result.size() << endl;
}

void SequentialAuction::printPath(int robot_num, vector<int> path) {
  cout << "[ ";
  for (int i = 0; i < path.size(); i++) {
    cout << path[i] << " ";
    alloction_result[robot_num].push_back(
        static_cast<double>(tasks[path[i]].pose.position.x));
    alloction_result[robot_num].push_back(
        static_cast<double>(tasks[path[i]].pose.position.y));
    // cout<<"\ntask["<<path[i]<<"]
    // position.x:"<<static_cast<double>(tasks[path[i]].pose.position.x);
    // cout<<"\ntask["<<path[i]<<"]
    // position.y:"<<static_cast<double>(tasks[path[i]].pose.position.y);
  }
  cout << "]" << endl;
}
